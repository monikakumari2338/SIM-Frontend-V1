...
export function DetailsTab({
   ...
   const details = {
      IA: [
         ...
         {
            label: "Total SKU",
            value: tempItems.length || entryItem.totalSku,
         },
      ],
      DSD: [
         {
      ...
      RTV: [
         ...
         {
            label: "Total SKU",
            value: entryItem.totalSku || tempItems.length,
         },
         {
            label: "Reason",
            value: entryItem.reason || tempReason,
   ...
   async function draftSc() {
      ...
      */
      ...
      const requestBody = {
         ...
         reason: tempReason,
         category: "Sportswear",
         items: tempItems.map(({ sku, upc, qty }) => ({
            sku,
            upc,
            qty,
         })),
            ...
            for (const [index, item] of data.entries()) {
               ...
               try {
                  const response = await getData(
                     endpoints.fetchItemBySku + `${item.SKU}/${storeName}`
                  );

                  console.log("RESPONSE", response);

                  const foundItem = response ? response.items[0] : null;
