...
export default function EntryItemDetailPage({ route }) {
  ...
  useEffect(() => {
    switch (type) {
      ...
      case "SC":
      case "RTV":
        getItemsReasonSupplier(); // RTV needs to be add
        break;
      case "TSFIN":
      case "TSFOUT":
        getTsfDetails();
        ...
        <FlatList
          ...
          ListHeaderComponent={() => (
            <>
              ...
              {status === "Complete" && type !== "PO" && (
                <SearchBar {...{ entryItem, tempItems, setTempItems }} />
              )}
              ...
              {status === "Partially Accepted" && (
                ...
                >
                  This is a partially accepted transfer. Please modify the item
          ...
          ListFooterComponent={
            <>
              {/* FAB Group */}
              {!isComplete &&
                !["Accepted", "Partially Accepted", "Rejected"].includes(
                  status
                ) && (
                  <MyFabGroup
                    {...{
              ...
              {type === "TSFOUT" && (
                <Portal>
                  <FAB
                    onPress={handleShip}
                    disabled={
                      !["Accepted", "Partially Accepted"].includes(status)
                    }
                    label="SHIP"
                    icon="truck-delivery"
                    style={{
    ...
    >
      ...
      {isStockCount ? (
        ...
        <>
          {/* Save Button */}
          <Button
            disabled={!canSaveOrSubmit || entryItem.status === "Dispatched"}
            title="Save"
            titleStyle={styles.buttonTitle}
            icon={{
              name: "content-save-outline",
      ...
      >
        {
          ...
          }[type].map(({ title, icon, iconType, onPress, enableStatus }) => (
            <Button
              key={title}
              title={title}
              titleStyle={
              ...
              onPress={onPress}
              disabled={
                !enableStatus.includes(status) || tempItems.length === 0
              }
            />
          ))
        }
